...
class Repository:
    ...
    def get_prs_created_per_day(self):
        with duckdb.connect() as conn:
            rel = conn.read_csv(self._uris)  # type: ignore
            created_at = duckdb.ColumnExpression("createdAt")
...
if __name__ == "__main__":
    ...
    base = root_uri + "/github/prs/opensafely-core"
    uris = [
        base + "/job-server/prs.csv",
        base + "/ethelred/prs.csv",
        base + "/airlock/prs.csv",
        base + "/job-runner/prs.csv",
    ]

    repository = Repository(uris)
    main(repository)
